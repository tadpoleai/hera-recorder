...
template<typename T>
void write_to_buffer(VectorBuffer& buffer, const T& data)
{
    buffer.insert(buffer.end(), data.begin(), data.end());
}

template<typename T>
void write_to_buffer(VectorBuffer& buffer, const T& data, size_t data_length)
{
...
template<typename T>
void write_ptr_to_buffer(VectorBuffer& buffer, const T* data, size_t data_length)
{
    buffer.insert(buffer.end(),
                  reinterpret_cast<const uint8_t*>(data),
                  reinterpret_cast<const uint8_t*>(data) + data_length);
...
inline void write_version(VectorBuffer& buffer)
{
...
inline uint32_t write_header(VectorBuffer& buffer, std::map<string, string> const& fields)
{
...
inline void write_file_header_record(VectorBuffer& buffer,
...
inline void start_writing(VectorBuffer& buffer)
{
    write_file_header_record(buffer, 0, 0, 0);
}

inline void write_connection_record(VectorBuffer& buffer, shared_ptr<ConnectionInfo>& connection)
{
    std::map<string, string> header;
...
inline void write_connection_records(VectorBuffer& buffer, std::map<uint32_t, shared_ptr<ConnectionInfo>>& connections)
{
...
inline void write_chunk_info_records(VectorBuffer& buffer, std::vector<ChunkInfo>& chunk_infos)
{
...
inline void stop_writing(VectorBuffer& buffer,
...
inline size_t write_chunk_header(VectorBuffer& buffer, uint32_t compressed_size, uint32_t uncompressed_size)
{
    ...
    header[rosbag::SIZE_FIELD_NAME] = impl::to_header_string(&chunk_header.uncompressed_size);
...
inline void generate_message_record_header(uint32_t conn_id, ros::Time const& time, ros::M_string& header)
{
    header[rosbag::OP_FIELD_NAME] = impl::to_header_string(&rosbag::OP_MSG_DATA);
...
inline size_t write_data_message_record_header(VectorBuffer& buffer, uint32_t conn_id, ros::Time const& time)
{
    ros::M_string header;
...
inline void write_index_records(VectorBuffer& buffer,
                                std::map<uint32_t, std::multiset<rosbag::IndexEntry>>& chunk_connection_indexes)
{
    for (auto& index_entry_pair : chunk_connection_indexes) {
        ...
        ros::M_string header;
        header[rosbag::OP_FIELD_NAME] = impl::to_header_string(&rosbag::OP_INDEX_DATA);
        header[rosbag::CONNECTION_FIELD_NAME] = impl::to_header_string(&connection_id);
        header[rosbag::VER_FIELD_NAME] = impl::to_header_string(&rosbag::INDEX_VERSION);
        header[rosbag::COUNT_FIELD_NAME] = impl::to_header_string(&index_size);
...
inline void write_chunk_end(VectorBuffer& buffer,
                            std::map<uint32_t, std::multiset<rosbag::IndexEntry>>& chunk_connection_indexes)
{
...
template<class T>
shared_ptr<rosbag::ConnectionInfo> DirectBag::get_connection_info(std::string const& topic,
                                                                  T const& msg,
                                                                  shared_ptr<ros::M_string> connection_header)
{
    ...
    if (!connection_header) {
        ...
        auto header_connection_id_pair = header_connection_ids_.find(connection_header_copy);
...
size_t write_data_message_record_header_with_padding(VectorBuffer& buffer,
                                                     uint32_t conn_id,
                                                     ros::Time const& time,
...
template<class T>
void DirectBag::write(std::string const& topic,
                      ros::Time const& time,
                      T const& msg,
                      shared_ptr<ros::M_string> connection_header)
{
    ...
    if (!is_open()) {
        throw rosbag::BagException("Tried to insert a message into a closed DirectBag.");
    }
    // Make sure a reasonable time is given
    if (time < ros::TIME_MIN) {
        throw rosbag::BagException("Tried to insert a message with time < ros::MIN_TIME");
    }
    ...
    auto connection_info = this->get_connection_info(topic, msg, connection_header);
    ...
    if (should_finish_chunk) {
        ...
        message_header_len = write_data_message_record_header_with_padding(chunk_buffer_,
                                                                           connection_info->id,
                                                                           time,
                                                                           file_->get_offset(),
                                                                           message_data_len,
                                                                           alignment_adjustment<T>(msg));
    } else {
        // Otherwise write the message header as normal, without extra padding
        message_header_len = write_data_message_record_header(chunk_buffer_, connection_info->id, time);
    }
    ...
    auto& chunk_connection_index = current_chunk_connection_indexes_[connection_info->id];
    ...
    if (should_finish_chunk) {
        ...
        std::copy(header_buffer.begin(), header_buffer.end(), chunk_buffer_.begin() + current_chunk_position_);
    }

    if (has_direct_data_bool) {
        SerializationReturnCode ret_code = SerializationReturnCode::SERIALIZE_TO_BUFFER_NEXT;
        size_t step = 0;
        ...
        ros::serialization::OStream s(chunk_buffer_.data() + current_position, message_data_len);
    ...
    if (should_finish_chunk) {
        ...
        chunk_buffer_.insert(chunk_buffer_.end(), chunk_end_buffer.begin(), chunk_end_buffer.end());
...
public:
    explicit BagFileException(std::string const& msg) : BagException(msg) {}
    BagFileException(const std::string& msg, int errnum) :
        BagFileException(msg + std::string(": ") + std::string(strerror(errnum)))
    {}
};
    ...
    for (auto& directory : directories) {
        ...
        if (!directory_exists(current_path)) {
            ...
            if (ret != 0) {
                ...
                throw BagFileException(err_msg, errno);
            }
        }
    }
}

inline std::string generate_bag_name(std::string& folder_path,
                                     ...
                                     size_t bag_number_width)
{
    ...
    ss << std::setfill('0') << std::setw(bag_number_width) << bag_number << ".bag";
    return ss.str();
}
...
void DirectBagCollection::write(Args&&... args)
{
    ...
    if (this->is_open()) {
        ...
        throw std::runtime_error("write called on a closed DirectBagCollection instance");
    }
}
