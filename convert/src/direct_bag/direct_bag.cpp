...
size_t chunk_header_length()
{
    ...
    static const size_t kchunk_header_length = write_chunk_header(__temp_constant_buffer, 0, 0);
                                                     ...
                                                     size_t alignment_adjustment)
{
    ...
    size_t projected_offset = current_file_offset + buffer.size() + 4  // length of message data length
                              + serialized_message_data_len - alignment_adjustment;
...
DirectBag::DirectBag(std::string filename, bool use_odirect, size_t chunk_threshold) : DirectBag()
{
...
void DirectBag::open(std::string filename, bool use_odirect, size_t chunk_threshold)
{
    if (this->is_open()) {
        throw std::runtime_error("open called on an already open DirectBag instance.");
    }
...
void DirectBag::close()
{
    ...
    if (!was_open) {
        throw std::runtime_error("close called on an already closed DirectBag instance");
    }
    ...
    if (stop_buffer_misalignment != 0) {
        stop_buffer.resize(stop_buffer.size() + (4096 - stop_buffer_misalignment), 0x00);
    }
    file_->write_buffer(stop_buffer);
    // Write the updated file header
    VectorBuffer file_header_buffer;
    impl::write_file_header_record(file_header_buffer, connections_.size(), chunk_infos_.size(), index_data_position);
    file_->seek(0);
...
DirectFile::DirectFile(const std::string& filename, bool use_odirect) :
    filename_(filename),
    open_(true),
    use_odirect_(use_odirect)
{
    ...
    const char* filename_ptr = filename_.c_str();
    int fd = open(filename_ptr, flags, mode);
    if (fd < 0) {
        throw BagFileException(std::string("Failed to open file: ") + filename, errno);
    }
    file_descriptor_ = fd;
}
...
std::vector<std::string> DirectBagCollection::close()
{
    ...
    for (size_t i = 1; i <= current_bag_number_; i++) {
        filenames.push_back(generate_bag_name(folder_path_, file_prefix_, i, bag_number_width_));
    }
    // Clean up
...
void DirectBagCollection::open_directory(const std::string& folder_path,
                                         bool use_odirect,
...
{
    if (this->is_open()) {
        throw std::runtime_error("open called on an already open DirectBagCollection instance.");
    }
    use_odirect_ = use_odirect;
...
{
    if (!this->is_open()) {
        throw std::runtime_error("get_chunk_threshold called on a closed DirectBagCollection instance");
    }
...
{
    if (!this->is_open()) {
        throw std::runtime_error("get_current_bag_file_name called on a closed "
...
void DirectBagCollection::open_next_bag_()
{
    ...
    current_bag_.reset(
            new DirectBag(generate_bag_name(folder_path_, file_prefix_, ++current_bag_number_, bag_number_width_),
                          use_odirect_,
                          chunk_threshold_));
}
