...
#include "direct_bag_dependencies.h"
...
public:
    ...
    DirectFile(const std::string& filename, bool use_odirect);
    ~DirectFile();
...
public:
    ...
    DirectBag(std::string filename, bool use_odirect, size_t chunk_threshold = kdefault_chunk_threshold);
     ...
     *                        defaults to 768kb.
     */
    void open(std::string filename, bool use_odirect, size_t chunk_threshold = kdefault_chunk_threshold);
...
private:
    template<class T>
    shared_ptr<rosbag::ConnectionInfo> get_connection_info(std::string const& topic,
                                                           T const& msg,
                                                           shared_ptr<ros::M_string> connection_header);

    template<class T>
    void write_message_data_record(uint32_t conn_id,
                                   ros::Time const& time,
                                   T const& msg,
...
public:
     ...
     */
    void open_directory(const std::string& folder_path,
                        bool use_odirect,
                        const std::string& file_prefix = kdefault_file_prefix,
...
template<class T>
bool has_direct_data(const T& msg)
{
...
template<class T>
size_t alignment_adjustment(const T& msg)
{
    ROSBAG_DIRECT_WRITE_UNUSED(msg);
    return 0;
}
...
public:
    explicit not_implemented_exception(const char* what_arg) : std::logic_error(what_arg) {}
};
...
template<class T>
SerializationReturnCode serialize_to_buffer(VectorBuffer& buffer, const T& msg, size_t serialization_step)
{
...
template<class T>
SerializationReturnCode serialize_to_file(DirectFile& file, const T& msg, size_t serialization_step)
{
