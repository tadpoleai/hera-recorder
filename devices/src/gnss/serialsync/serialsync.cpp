...
///
StorageDataPtr Serialsync::fetch()
{
    ...
    try {
        ...
        if (token.size() != 0) {
            ...
            uint64_t timestamp_nmea_original =
                    hours * OneHour + minutes * OneMinute + seconds * OneSecond + nanoseconds;

            if (now - shiftation_timestamp_ < ShifationDelayTolerance_) {
                timestamp_nmea_original += shifation_value_ns_;

                uint64_t days = now / OneDay;
                int64_t now_residual = int64_t(now) - days * OneDay;

                if (int64_t(timestamp_nmea_original) - now_residual > OneDay / 2) {
                    days -= 1;
                } else if (int64_t(timestamp_nmea_original) - now_residual < -OneDay / 2) {
                    days += 1;
                }

                timestamp_aligned_valid = 1;
                timestamp_aligned = timestamp_nmea_original + days * OneDay;
            }
        }
    ...
    auto length = sizeof(SerialsyncNmeaStorageData) + nmea_sentence_length;
    auto data = StorageData::create(length,
                                    DeviceVendorType::GnssSerialSync,
                                    StorageDataType::GnssSerialsyncNmea,
                                    sequence_++);
    auto derived_data = static_cast<SerialsyncNmeaStorageData*>(data.get());
...
SensorDataPtr Serialsync::convert(StorageDataPtr&& storage_data)
{
    ...
    auto raw_data = static_cast<SerialsyncNmeaStorageData*>(storage_data.get());

    // Create a SensorData from StorageData
    auto length = sizeof(NavSatFixSensorData);
    auto sensor_data = SensorData::create_from(storage_data, SensorDataType::NavSatFix, length);
    auto navsatfix_sensor_data = static_cast<NavSatFixSensorData*>(sensor_data.get());


    // Initialize an invalid data template
    navsatfix_sensor_data->timestamp_intrinsic_ns = raw_data->data.timestamp_intrinsic_ns;
    navsatfix_sensor_data->status.status = StatusType::NO_Fix;
    navsatfix_sensor_data->status.service = ServiceType::GPS;
    navsatfix_sensor_data->position_covariance_type = PositionCovarianceType::Unknown;
    ...
    // Return if timestamp is not valid
    ...
    try {
        ...
        switch (token[0]) {
        case '4':
            fixed = StatusType::FIX;
            break;
        default:
            fixed = StatusType::NO_Fix;
            break;
        }
