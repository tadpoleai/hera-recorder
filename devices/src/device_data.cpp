...
///
StorageDataPtr StorageData::create(uint32_t length)
{
...
///
StorageDataPtr StorageData::create(uint32_t length, DeviceVendorType type, StorageDataType msgtype, uint32_t sequence)
{
...
StorageDataPtr StorageData::read_from(std::ifstream& ifs)
{
    try {
        uint32_t length;
...
size_t StorageData::write_to(std::ofstream& ofs) const
{
    try {
...
DisplayDataPtr DisplayData::broken_data()
{
    ...
    disp_data->timestamp_intrinsic_ns = Timestamp::now();
    disp_data->data = "";
