...
void Service::deleteStorage(std::vector<StorageRecordFile>& result, const std::vector<std::string>& names)
{
    std::unique_lock<std::mutex> _(mutex_);

    auto folder = file::get_folder_content(DataDirectory_);
    for (auto& file : folder.files) {
        if (std::find(names.begin(), names.end(), file.basename) != names.end()) {
    ...
    for (auto& file : folder.files) {
        ...
        try {
            for (size_t i = 0; i < header->device_names.size(); ++i) {
                ...
                info.devices[i].dataSize = header->device_data_sizes[i];
            }
        } catch (...) {
        };
