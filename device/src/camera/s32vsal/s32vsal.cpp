...
const std::vector<DeviceParameterType> S32VSal::OptionalParameterTypes = {DeviceParameterType::FrameRate,
                                                                          DeviceParameterType::Compress,
                                                                          DeviceParameterType::CompressQuality};
...
HeraErrno S32VSal::connect()
{
    ...
    if (parameters_.count(DeviceParameterType::FrameRate)) {
        try {
            frame_rate_ = stoi(parameters_[DeviceParameterType::FrameRate]);
            last_time_ = time::Timestamp::now();
            log::info << "CameraS32VSal: Set FrameRate to '" << frame_rate_ << "'" << log::endl;
        } catch (...) {
            log::warn << "CameraS32VSal: Can not parse parameter FrameRate: '"
                      << parameters_[DeviceParameterType::FrameRate] << "', ignored" << log::endl;
            frame_rate_ = 0;
        }
    }

    if (parameters_.count(DeviceParameterType::Compress)) {
        try {
            compress_ = (stoi(parameters_[DeviceParameterType::Compress]) > 0);
        } catch (...) {
            compress_ = false;
        }
    }
    ...
    if (compress_) {
        if (parameters_.count(DeviceParameterType::CompressQuality)) {
            try {
                compress_quality_ = stoi(parameters_[DeviceParameterType::CompressQuality]);
