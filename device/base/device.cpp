...
HeraErrno Device::start()
{
    ...
    case DeviceStatus::BeforeConnect: {
        if (!check_parameter()) {
            return handle_error(HeraErrno::InsufficientParameters, "Insufficient Parameters");
        }
        auto result = connect();
    ...
    case DeviceStatus::Connected:
        return handle_error(HeraErrno::DeviceAlreadyConnected, "Device is already connected");

    case DeviceStatus::Terminated:
        return handle_error(HeraErrno::DeviceAlreadyClosed, "Device is already closed");

    case DeviceStatus::Error:
    default:
        return handle_error(HeraErrno::InStatusError, "Device is in status error");
    }
}
...
HeraErrno Device::record(const bool value)
{
    switch (status_) {
    case DeviceStatus::BeforeConnect:
