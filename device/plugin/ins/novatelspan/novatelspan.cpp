...
const std::vector<std::string> NovatelSpan::EssentialParameterTypes = {"Kernel",
                                                                       "KernelAuxiliary",
                                                                       "BaudRate",
                                                                       "BaudRateAuxiliary",
                                                                       "SerialMsgType"};

const std::vector<std::string> NovatelSpan::OptionalParameterTypes = {};

HERA_DEVICE_DRIVER_EXPORT_PLUGIN(InsNovatelSpan, "ins/novatelspan", NovatelSpan)
...
HeraErrno NovatelSpan::connect()
{
    try {
        ...
        kernel_auxiliary_ = parameters_["KernelAuxiliary"];
        baud_rate_ = stoi(parameters_["BaudRate"]);
        baud_rate_auxiliary_ = stoi(parameters_["BaudRateAuxiliary"]);
        serial_msg_type_ = stoi(parameters_["SerialMsgType"]);
    } catch (...) {
