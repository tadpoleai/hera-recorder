...
const std::vector<std::string> S32VGeely::EssentialParameterTypes = {"DataPort"};

const std::vector<std::string> S32VGeely::OptionalParameterTypes = {};

HERA_DEVICE_DRIVER_EXPORT_PLUGIN(OdometryS32VGeely, "odometry/s32vgeely", S32VGeely)
...
HeraErrno S32VGeely::connect()
{
    ...
    try {
        port_num = stoi(parameters_["DataPort"]);
    } catch (const std::exception& e) {
        return handle_error(HeraErrno::InvalidParameterValue,
                            "S32VGeely:: invalid data port" + parameters_["DataPort"]);
    }
