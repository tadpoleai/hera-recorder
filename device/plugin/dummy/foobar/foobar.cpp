...
const std::vector<std::string> Foobar::EssentialParameterTypes = {"DummyRate", "DummyValue"};

const std::vector<std::string> Foobar::OptionalParameterTypes = {};

HERA_DEVICE_DRIVER_EXPORT_PLUGIN(DummyFoobar, "dummy/foobar", Foobar);
...
{
    try {
        int_value_ = stoi(parameters_["DummyValue"]);
    ...
    if (parameters_.count("DummyString")) {
        string_message_ = "DummyString";
    } else {
...
HeraErrno Foobar::adjust_parameter(const std::string& type, const std::string& value)
{
    if (type == "DummyValue") {
        try {
