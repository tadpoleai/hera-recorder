...
const std::vector<std::string> Velodyne::EssentialParameterTypes = {"IpAddress", "DataPort"};

const std::vector<std::string> Velodyne::OptionalParameterTypes = {"RotationalSpeed", "SyncInvalid"};

HERA_DEVICE_DRIVER_EXPORT_PLUGIN(LidarVelodyne, "lidar/velodyne", Velodyne)
...
{
    log::debug << "Velodyne:: Connecting to velodyne by binding port: " << parameters_["DataPort"] << log::endl;

    rotational_speed_ = NominalRPM_;
    if (parameters_.count("RotationalSpeed") && !parameters_["RotationalSpeed"].empty()) {
        try {
            rotational_speed_ = std::stoi(parameters_["RotationalSpeed"]);
    ...
    if (rotational_speed_ < MinimumRPM_) {
        return handle_error(HeraErrno::InvalidParameterValue,
                            "Velodyne::" + get_name() +
                                    "Invalid Parameter RotationalSpeed = " + parameters_["RotationalSpeed"] +
    ...
    if (parameters_.count("SyncInvalid") && !parameters_["SyncInvalid"].empty()) {
        sync_invalid_ = true;
    ...
    int ret = system(("curl -m 0.2 --data \"motor=on&laser=on&rpm=" + std::to_string(rotational_speed_) + "\" http://" +
                      parameters_["IpAddress"] + "/cgi/setting")
                             .c_str());
    if (ret == 0) {
    ...
    try {
        ...
        addr_in_.sin_port = htons(std::stoi(parameters_["DataPort"]));
...
void Velodyne::disconnect()
{
    ...
    int ret = system(
            ("curl -m 0.2 --data \"motor=off&laser=off&rpm=300\" http://" + parameters_["IpAddress"] + "/cgi/setting")
                    .c_str());
    if (ret == 0) {
