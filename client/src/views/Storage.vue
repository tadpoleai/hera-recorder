...
.storage
  transition(name='van-fade')
    van-cell-group(v-if='uploadProcesses.length', title='上传进度')
      transition(name='van-fade', v-for='p in uploadProcesses')
        UploadProcess(:process='p')

  van-cell-group(title='储存容量')
  ...
  van-cell-group
    div(slot='title', style='display: flex; justify-content: space-between')
      span 机内数据
      span
        template(v-if='isMultiSelectionMode')
    ...
    template(v-if='isMultiSelectionMode')
      van-cell(v-for='(storage, index) in fetchedData', center, @click='onClickStorageData(storage.name)')
    ...
    van-collapse(v-else, v-model='selectedSingleStorageData', accordion)
      //- For storage record files
      van-collapse-item(v-for='(storage, index) in fetchedData', :title='storage.name')
        div(slot='title', style='display: flex; justify-content: space-between')
          van-tag(plain, type='primary', style='margin-top: 4px')
            | {{ storage.name }}
          van-tag(plain, type='success', style='margin-top: 4px')
        ...
        van-cell
          van-row(style='margin: 1em 0 0 0')
            van-col.center(span='12')
              van-button(type='danger', size='small', @click='onClickDeleteSingle(storage.name)') 删除
            van-col.center(span='12')
              van-button(type='primary', size='small', @click='onClickUploadSingle(storage.name)') 上传
</template>
