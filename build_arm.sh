...
if [ -z "$1" ]; then
    cmake .. -Dwith-driver-arm=1 -Dtarget-arm=1 -DCMAKE_INSTALL_PREFIX=/opt/s32v/hera
else
    echo "git version is $1"
    cmake .. -Dwith-driver-arm=1 -Dtarget-arm=1 -DCMAKE_INSTALL_PREFIX=/opt/s32v/hera -Dforce-git-info=$1
fi
...
cp -r device/hera-device-s32vmipi-test ../artifacts/arm/bin
