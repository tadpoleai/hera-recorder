...
AccumulatorVelodyne::AccumulatorVelodyne(ros::NodeHandle& node,
                                                     ros::NodeHandle& privateNode)
{
    ...
    cloud_accumulate_thread_ =
            std::thread(&AccumulatorVelodyne::accumulated_points_thread, this);

    //parameter 04:accumulated_topic
    accumulated_topic_ = private_nh_.param<std::string>("accumulated_topic", "accumulated_points");
    accumulated_points_pub_ =
            nh_.advertise<sensor_msgs::PointCloud2>(accumulated_topic_, 10, false);
}
...
void AccumulatorVelodyne::original_points_callback(
...
void AccumulatorVelodyne::accumulated_points_thread()
{
    ...
    std::for_each( point_scan.begin(), point_scan.end(),
