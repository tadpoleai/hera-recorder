...
Pointcloud2LaserScanVelodyne::Pointcloud2LaserScanVelodyne(ros::NodeHandle& node, ros::NodeHandle& privateNode)
{
    ...
    laserscan_pub_ = nh_.advertise<sensor_msgs::LaserScan>(laserscan_output_topic_, 10, false);
}
...
void Pointcloud2LaserScanVelodyne::pointcloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
    ...
    uint32_t ranges_size =
            std::ceil((output_laserscan.angle_max - output_laserscan.angle_min) / output_laserscan.angle_increment);
    ...
    for (sensor_msgs::PointCloud2ConstIterator<float> iter_x(*transformed_cloud, "x"),
        ...
        if (h_angle < output_laserscan.angle_min || h_angle > output_laserscan.angle_max) {
            ROS_DEBUG("rejected for angle %f not in range (%f, %f)\n",
                      h_angle,
                      output_laserscan.angle_min,
                      output_laserscan.angle_max);
            continue;
        }
